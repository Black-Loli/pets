...
<div class="left_menu">
    ...
    <ul>
        ...
        <li>
            <input type="radio" name="profile_selector" id="response_con" class="con_selector">
            <label for="response_con" class="con_selector_label">
                Отклики
            </label>
        </li>

        <li>
            <input type="radio" name="profile_selector" id="my_order_con" class="con_selector">
            <label for="my_order_con" class="con_selector_label">
                Расписание
            </label>
        </li>
...
<div class="tab-content">
    <div id="order_con_content" class="content active">
        <div class="orders">
            ...
            foreach ($questionnaire as $questionnairePet) {
                ...
                $response_header = $bd->query("SELECT *
                    FROM Response_Executors
                             LEFT JOIN Executor on Executor.ID = Response_Executors.ID_Executor
                    WHERE Response_Executors.ID_Service =  $questionnairePet->ServiceID")->fetch(PDO::FETCH_OBJ);
                if ($response_header->ID_Service == $questionnairePet->ServiceID) {
                    echo "<h1>На рассмотрении</h1>";
                } else {
                    echo "<form action='/reserve_handler.php' method='post'>
                        <input type='hidden' name='ID_Service' required='' value='{$questionnairePet->ServiceID}'>
                        <input type='submit' name='response' class='btn' required=''>
                      </form>";
                }
    ...
    <div id="response_con_content" class="content">
        <div class="response">
            <?php
            $user_id = $_SESSION['user_id'];
            $questionnaire = $bd->query("SELECT *,
                ...
                FROM Service
                         ...
                         LEFT JOIN Executor on Executor.ID = Response_Executors.ID_Executor
                WHERE Executor.ID_user = $user_id and Service.ID_Executor is null
                ORDER BY DateTimeStart")->fetchAll(PDO::FETCH_OBJ);
            ...
            foreach ($questionnaire as $questionnairePet) {
                ...
                echo "<div class='application__footer'>";
                echo "<h1>в $questionnairePet->TimeStart, $questionnairePet->Address</h1>";
                echo "<a id='modal_open_response_order' data-pet-id='{$questionnairePet->ID_Pet}' data-order_response-id='{$questionnairePet->ServiceID}'>Смотреть подробнее</a>";
                echo "</div>";
                echo "</div>";
            }
            ?>
    ...
    <div id="my_order_con_content" class="content">
        <div class="my_orders orders">
            <?php
            $user_id = $_SESSION['user_id'];
            $questionnaire = $bd->query("SELECT *,
                   DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
            ...
            <div class="overlay_modal" id="overlay_modal_my_order"></div>
            <div class="modal" id="modal_my_order">
                <div class="modal_header">
                    <h1>Заказ на питомца</h1>
                    <svg id="modal_close_my_order" viewBox="0 0 43 43" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M41.3334 1.3335L1.33337 41.3335M1.33342 1.3335L41.3334 41.3335" stroke="#006270"
                              stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </div>

                <div class="modal_body modal_body_my_order">
                    <?php
                    $data = $bd->query("Select *,
                           DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
                           DATE_FORMAT(DATE(DateTimeStart), '%d %M %Y') AS DateStart,
                    ...
                    FROM Pets
                             ...
                             LEFT JOIN Type_Service on Type_Service.ID = Service.Service_Type
                             LEFT JOIN Executor on Executor.ID = Service.ID_Executor
                    WHERE Executor.ID_user = $user_id")->fetchAll(PDO::FETCH_OBJ);
                    $order_question = array();
                    foreach ($data as $element) {
                        $order_question[$element->ServiceID][] = $element;
                    }
                    foreach ($order_question as $ServiceID => $orders) {
                        echo "<div class='pet_order' id='order_$ServiceID' style='display: none'>";
                        echo "<p>№ {$orders[0]->ServiceID}</p>";
                        echo "<h1>{$orders[0]->DateStart}</h1>";
                        echo "<h1>{$orders[0]->Description} {$orders[0]->TimeStart}</h1>";
                        echo "<h1>{$orders[0]->Name}</h1>";
...
<script>
    ...
    $('#modal_open_my_order,#overlay_modal_my_order,#modal_close_my_order').click(function (event) {
        $('#overlay_modal_my_order').toggleClass("active")
        $('#modal_my_order').toggleClass("active")
        console.log(event.target.dataset.ordermyId)
        if (event.target.id === 'modal_open_my_order') {
            $('.modal_body_my_order div.pet_order').hide();
            // console.log($(`.modal_body_my_order #order_${event.target.dataset.ordermyId}`))
            $(`.modal_body_my_order #order_${event.target.dataset.ordermyId}`).show();
        }
    });
    ...
    $('#modal_open_response_order,#overlay_modal_response_order,#modal_close_response_order').click(function (event) {
        ...
        if (event.target.id === 'modal_open_response_order') {
            ...
            // console.log($(`.modal_body_response_order #response_${event.target.dataset.ordermyId}`))
            $(`.modal_body_response_order #response_${event.target.dataset.order_responseId}`).show();
        }
    });
