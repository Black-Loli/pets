...
$applicationQuery = $bd->query("SELECT Numder, Applications.Name as AppName, Name_Service.Name as ServName FROM Applications LEFT JOIN Name_Service on Applications.ID_Service_Name = Name_Service.ID");
$applicationQueryResult = $applicationQuery->fetchAll(PDO::FETCH_OBJ);
?>
<?php
...
<main>
    <div class="left_menu">
        <div class="person">
            ...
            <div>
                <?php
                echo "<h1>$userQueryResult->Surname</h1> <h1>$userQueryResult->Name</h1>
        ...
        <ul>
            ...
            <li>
                <input type="radio" name="profile_selector" id="order_response_con" class="con_selector">
                <label for="order_response_con" class="con_selector_label">
                    Заказы с откликами
                </label>
            </li>

            <li>
                <input type="radio" name="profile_selector" id="order_executor_con" class="con_selector">
                <label for="order_executor_con" class="con_selector_label">
                    Заказы с исполнителем
                </label>
            </li>

            <li>
                <input type="radio" name="profile_selector" id="call_con" class="con_selector">
                <label for="call_con" class="con_selector_label">
                    Заявки на звонок
                </label>
            </li>
    ...
    <div class="tab-content">
        <div id="order_con_content" class="content active">
            <div class="orders">
                ...
                $questionnaire = $bd->query("SELECT *,
                           ...
                           Response_Executors.ID                        as Response_Executors_ID
                    FROM Service
                             LEFT JOIN Type_Service on Type_Service.ID = Service.Service_Type
                             LEFT JOIN Pets on Pets.ID = Service.ID_Pet
                             LEFT JOIN Response_Executors on Service.ID = Response_Executors.ID_Service
                    WHERE Service.ID_Executor is null
                ...
                foreach ($questionnaire as $questionnairePet) {
                    ...
                    $result = $bd->query("SELECT COUNT(ID_Service) as count
                    FROM Response_Executors
                             LEFT JOIN Executor on Executor.ID = Response_Executors.ID_Executor
                    WHERE ID_Service = $questionnairePet->ServiceID ")->fetch(PDO::FETCH_OBJ);
                    if ($result->count == 0) {
                        echo "<h3>Ждём отклики</h3>";
                    } else {
                        echo "<a id='modal_open_response_order' data-response-id='{$questionnairePet->ServiceID}'>Смотреть отклики</a>";
                    }
                    echo "</div>";
                    echo "<div class='application__footer'>";
                    echo "<h1>в $questionnairePet->TimeStart, $questionnairePet->Address</h1>";
                    echo "<a id='modal_open_order' data-pet-id='{$questionnairePet->ID_Pet}' data-order-id='{$questionnairePet->ServiceID}'>Смотреть подробнее</a>";
                    echo "</div>";
                    echo "</div>";
                }
                ?>

                <div class="overlay_modal" id="overlay_modal_response_order"></div>
                <div class="modal" id="modal_response_order">
                    ...
                    <div class="modal_body modal_body_response_order">
                        <?php
                        $data = $bd->query("SELECT *,
                            ...
                            FROM Response_Executors
                                     LEFT JOIN Executor on Executor.ID = Response_Executors.ID_Executor
                                     LEFT JOIN Users on Users.ID = Executor.ID_user
                                     LEFT JOIN Service on Service.ID = Response_Executors.ID_Service
                        ...
                        foreach ($order_question as $ServiceID => $orders) {
                            echo "<div class='pet_order' id='response_$ServiceID' style='display: none'>";
                            echo "<p>Заказ №{$orders[0]->ServiceID}</p>";
                            foreach ($orders as $order) {
                                echo "<div class='pet_order_response'>";
                                echo "<h1>{$order->Surname} {$order->Name} {$order->Patronymic}</h1>";
                                echo "<form action='/purpose_handler.php' method='post'>
                                        <input type='hidden' name='ID_Service' required='' value='{$order->ServiceID}'>
                                        <input type='hidden' name='ID_Executor' required='' value='{$order->ExecutorID}'>
                                        <input type='submit' name='response' class='btn' required='' value='Назначить'>
                                      </form>";
                                echo "</div>";
                            }
                            echo "</div>";
                ...
                <div class="modal" id="modal_order">
                    ...
                    <div class="modal_body modal_body_order">
                        ...
                        FROM Pets
                                ...
                                LEFT JOIN Type_Service on Type_Service.ID = Service.Service_Type
                                WHERE Service.ID > 0")->fetchAll(PDO::FETCH_OBJ);
                        $order_question = array();
                        foreach ($data as $element) {
                            $order_question[$element->ServiceID][] = $element;
                        }
        ...
        <div id="order_response_con_content" class="content">
            <div class="orders">
                <?php
                $questionnaire = $bd->query("SELECT *,
                           DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
                    ...
                    FROM Service
                             ...
                             LEFT JOIN Response_Executors on Service.ID = Response_Executors.ID_Service
                    WHERE Service.ID_Executor IS null
                      and Response_Executors.ID IS NOT NULL
                    ORDER BY DateTimeStart")->fetchAll(PDO::FETCH_OBJ);
                ...
                foreach ($questionnaire as $questionnairePet) {
                    ...
                    FROM Response_Executors
                             LEFT JOIN Executor on Executor.ID = Response_Executors.ID_Executor
                    WHERE ID_Service = $questionnairePet->ServiceID ")->fetch(PDO::FETCH_OBJ);
                    echo "<a id='modal_open_order_response' data-response-id='{$questionnairePet->ServiceID}'>Смотреть отклики</a>";
                    echo "</div>";
                    echo "<div class='application__footer'>";
                    echo "<h1>в $questionnairePet->TimeStart, $questionnairePet->Address</h1>";
                    echo "<a id='modal_open_order_pet' data-pet-id='{$questionnairePet->ID_Pet}' data-order-id='{$questionnairePet->ServiceID}'>Смотреть подробнее</a>";
                    echo "</div>";
                    echo "</div>";
                }
                ?>

                <div class="overlay_modal" id="overlay_modal_order_response"></div>
                <div class="modal" id="modal_order_response">
                    <div class="modal_header">
                        <h1>Заказ на питомца</h1>
                        <svg id="modal_close_order_response" viewBox="0 0 43 43" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path d="M41.3334 1.3335L1.33337 41.3335M1.33342 1.3335L41.3334 41.3335" stroke="#006270"
                                  stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    ...
                    <div class="modal_body modal_body_order_response">
                        <?php
                        $data = $bd->query("SELECT *,
                                   Service.ID  as ServiceID,
                                   Executor.ID  as ExecutorID
                        ...
                        foreach ($order_question as $ServiceID => $orders) {
                            echo "<div class='pet_order' id='order_response_$ServiceID' style='display: none'>";
                            echo "<p>Заказ №{$orders[0]->ServiceID}</p>";
                            foreach ($orders as $order) {
                                echo "<div class='pet_order_response'>";
                                echo "<h1>{$order->Surname} {$order->Name} {$order->Patronymic}</h1>";
                ...
                <div class="overlay_modal" id="overlay_modal_order_pet"></div>
                <div class="modal" id="modal_order_pet">
                    <div class="modal_header">
                        <h1>Заказ на питомца</h1>
                        <svg id="modal_close_order_pet" viewBox="0 0 43 43" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M41.3334 1.3335L1.33337 41.3335M1.33342 1.3335L41.3334 41.3335" stroke="#006270"
                                  stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </div>

                    <div class="modal_body modal_body_order_pet">
                        <?php
                        $data = $bd->query("Select *,
                                DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
                                DATE_FORMAT(DATE(DateTimeStart), '%d %M %Y') AS DateStart,
                        ...
                        foreach ($order_question as $ServiceID => $orders) {
                            echo "<div class='pet_order' id='order_pet_$ServiceID' style='display: none'>";
                            echo "<p>Заказ №{$orders[0]->ServiceID}</p>";
                            echo "<h1>{$orders[0]->DateStart}</h1>";
                            echo "<h1>{$orders[0]->Description} {$orders[0]->TimeStart}</h1>";
                            echo "<h1>{$orders[0]->Name}</h1>";
        ...
        <div id="order_executor_con_content" class="content">
            <div class="orders">
                <?php
                $questionnaire = $bd->query("SELECT *,
                           DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
                ...
                foreach ($questionnaire as $questionnairePet) {
                    ...
                    echo "<div class='application__footer'>";
                    echo "<h1>в $questionnairePet->TimeStart, $questionnairePet->Address</h1>";
                    echo "<a id='modal_open_pet_order' data-pet-id='{$questionnairePet->ID_Pet}' data-order-id='{$questionnairePet->ServiceID}'>Смотреть подробнее</a>";
                    echo "</div>";
                    echo "</div>";
                }
                ?>

                <div class="overlay_modal" id="overlay_modal_pet_order"></div>
                <div class="modal" id="modal_pet_order">
                    <div class="modal_header">
                        <h1>Заказ на питомца</h1>
                        <svg id="modal_close_pet_order" viewBox="0 0 43 43" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M41.3334 1.3335L1.33337 41.3335M1.33342 1.3335L41.3334 41.3335" stroke="#006270"
                                  stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </div>

                    <div class="modal_body modal_body_pet_order">
                        <?php
                        $data = $bd->query("Select *,
                                DATE_FORMAT(TIME(DateTimeStart), '%H:%i')    AS TimeStart,
                                DATE_FORMAT(DATE(DateTimeStart), '%d %M %Y') AS DateStart,
                        ...
                        foreach ($order_question as $ServiceID => $orders) {
                            echo "<div class='pet_order' id='pet_order_$ServiceID' style='display: none'>";
                            echo "<p>Заказ №{$orders[0]->ServiceID}</p>";
                            echo "<h1>{$orders[0]->DateStart}</h1>";
                            echo "<h1>{$orders[0]->Description} {$orders[0]->TimeStart}</h1>";
                            echo "<h1>{$orders[0]->Name}</h1>";
        ...
        <div id="call_con_content" class="content">
            <div class="rez">
                <?php
                foreach ($applicationQueryResult as $application) {
                    echo "<div class='application'>";
                    echo "<h1>$application->AppName</h1>";
                    echo "<h1>$application->Numder</h1>";
                    echo "<h1>$application->ServName</h1>";
                    echo "</div>";
                    echo "\n\r";
                }
                ?>
...
<script>
    ...
    $('#modal_open_response_order,#overlay_modal_response_order,#modal_close_response_order').click(function (event) {
        $('#overlay_modal_response_order').toggleClass("active")
        $('#modal_response_order').toggleClass("active")
        if (event.target.id === 'modal_open_response_order') {
            $('.modal_body_response_order div.pet_order').hide();
            $(`.modal_body_response_order div#response_${event.target.dataset.responseId}`).show();
        }
    });

    $('#modal_open_order_response,#overlay_modal_order_response,#modal_close_order_response').click(function (event) {
        $('#overlay_modal_order_response').toggleClass("active")
        $('#modal_order_response').toggleClass("active")
        if (event.target.id === 'modal_open_order_response') {
            $('.modal_body_order_response div.pet_order').hide();
            $(`.modal_body_order_response div#order_response_${event.target.dataset.responseId}`).show();
        }
    });

    $('#modal_open_order_pet,#overlay_modal_order_pet,#modal_close_order_pet').click(function (event) {
        $('#overlay_modal_order_pet').toggleClass("active")
        $('#modal_order_pet').toggleClass("active")
        if (event.target.id === 'modal_open_order_pet') {
            $('.modal_body_order_pet div.pet_order').hide();
            $(`.modal_body_order_pet div#order_pet_${event.target.dataset.orderId}`).show();
        }
    });

    $('#modal_open_pet_order,#overlay_modal_pet_order,#modal_close_pet_order').click(function (event) {
        $('#overlay_modal_pet_order').toggleClass("active")
        $('#modal_pet_order').toggleClass("active")
        if (event.target.id === 'modal_open_pet_order') {
            $('.modal_body_pet_order div.pet_order').hide();
            $(`.modal_body_pet_order div#pet_order_${event.target.dataset.orderId}`).show();
        }
    });
